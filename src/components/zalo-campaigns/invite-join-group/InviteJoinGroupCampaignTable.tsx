...
interface InviteJoinGroupCampaignTableProps {
  ...
  onEdit: (campaign: InviteJoinGroupCampaign) => void;
  onCopy: (campaign: InviteJoinGroupCampaign) => void;
  onResults: (campaign: InviteJoinGroupCampaign) => void;
  onDelete: (campaign: InviteJoinGroupCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function InviteJoinGroupCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  onResults,
  onDelete,
}: InviteJoinGroupCampaignTableProps) {
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản mời bạn vào nhóm. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
            ...
            {campaigns.map((campaign, index) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="px-4 py-3 text-theme-sm text-gray-600 dark:text-gray-300">
                    {campaign.friend_count
                      ? `${campaign.friend_count} người`
                      : "Chưa có bạn bè được gán"}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-theme-sm text-gray-600 dark:text-gray-300">
