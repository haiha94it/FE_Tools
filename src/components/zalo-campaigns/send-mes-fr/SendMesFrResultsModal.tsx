...
import type { SendMesFrCampaignStatistics } from "@/types/zalo-send-mes-fr-campaign";
import type { ZaloAccount } from "@/types/zalo-account";
import Image from "next/image";

interface SendMesFrResultsModalProps {
  open: boolean;
...
function buildResultStats(statistics: SendMesFrCampaignStatistics, resultsTotal: number) {
  return [
    {
      label: "Thành công",
      value: statistics.mess_friend_success ?? statistics.success ?? 0,
      valueClass: "text-success-600 dark:text-success-400",
    },
    {
      label: "Thất bại",
      value: statistics.mess_friend_failure ?? statistics.failed ?? 0,
...
export default function SendMesFrResultsModal({
  open,
  accounts,
  campaignName,
  onClose,
}: SendMesFrResultsModalProps) {
  const results = useZaloSendMesFrCampaignStore((s) => s.results);
  const resultsSelectedIds = useZaloSendMesFrCampaignStore((s) => s.resultsSelectedIds);
  const resultsPage = useZaloSendMesFrCampaignStore((s) => s.resultsPage);
  const resultsPerPage = useZaloSendMesFrCampaignStore((s) => s.resultsPerPage);
  const resultsTotal = useZaloSendMesFrCampaignStore((s) => s.resultsTotal);
  const resultsLoading = useZaloSendMesFrCampaignStore((s) => s.resultsLoading);
  const statistics = useZaloSendMesFrCampaignStore((s) => s.statistics);
  const toggleResultSelected = useZaloSendMesFrCampaignStore((s) => s.toggleResultSelected);
  const toggleSelectAllResults = useZaloSendMesFrCampaignStore((s) => s.toggleSelectAllResults);
  const setResultsPage = useZaloSendMesFrCampaignStore((s) => s.setResultsPage);
  const setResultsPerPage = useZaloSendMesFrCampaignStore((s) => s.setResultsPerPage);
  const deleteSelectedResults = useZaloSendMesFrCampaignStore((s) => s.deleteSelectedResults);
                    ...
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="px-4 py-3 text-theme-sm">
                          {(() => {
                            const status = formatSendMesFrResultStatus(row.status);
                            return (
