...
interface SendMesFrCampaignTableProps {
  ...
  onEdit: (campaign: SendMesFrCampaign) => void;
  onCopy: (campaign: SendMesFrCampaign) => void;
  onResults: (campaign: SendMesFrCampaign) => void;
  onDelete: (campaign: SendMesFrCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function SendMesFrCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản nhắn tin bạn bè. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
