...
import type { SendMessMemberGrCampaign } from "@/types/zalo-send-mess-member-gr-campaign";

interface SendMessMemberGrCampaignTableProps {
  ...
  onEdit: (campaign: SendMessMemberGrCampaign) => void;
  onCopy: (campaign: SendMessMemberGrCampaign) => void;
  onResults: (campaign: SendMessMemberGrCampaign) => void;
  onDelete: (campaign: SendMessMemberGrCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function SendMessMemberGrCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  onResults,
  onDelete,
}: SendMessMemberGrCampaignTableProps) {
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản tương tác nhóm. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
