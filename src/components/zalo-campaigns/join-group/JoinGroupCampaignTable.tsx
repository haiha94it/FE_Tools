...
interface JoinGroupCampaignTableProps {
  ...
  onEdit: (campaign: JoinGroupCampaign) => void;
  onCopy: (campaign: JoinGroupCampaign) => void;
  onResults: (campaign: JoinGroupCampaign) => void;
  onDelete: (campaign: JoinGroupCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function JoinGroupCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản tham gia nhóm. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
            ...
            {campaigns.map((campaign, index) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="px-4 py-3 text-theme-sm text-gray-600 dark:text-gray-300">
                    {campaign.list_group_count
                      ? `${campaign.list_group_count} nhóm`
                      : "Chưa có link nhóm"}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-theme-sm text-gray-600 dark:text-gray-300">
