...
import {
  ...
  formatSendMesGroupCampaignRunStatus,
...
interface SendMesGroupCampaignTableProps {
  ...
  onEdit: (campaign: SendMesGroupCampaign) => void;
  onCopy: (campaign: SendMesGroupCampaign) => void;
  onResults: (campaign: SendMesGroupCampaign) => void;
  onDelete: (campaign: SendMesGroupCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function SendMesGroupCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  onResults,
  onDelete,
}: SendMesGroupCampaignTableProps) {
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản nhắn tin nhóm. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {campaigns.map((campaign, index) => {
              const status = resolveCampaignStatusDisplay(campaign, formatSendMesGroupCampaignRunStatus);
              const groupCount = campaign.group_count ?? 0;
              return (
                <TableRow
                  key={campaign.id}
