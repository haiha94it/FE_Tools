...
import {
  ...
  formatSendMesGroupResultStatus,
...
import type { SendMesGroupCampaignStatistics } from "@/types/zalo-send-mes-group-campaign";
import type { ZaloAccount } from "@/types/zalo-account";
import Image from "next/image";

interface SendMesGroupResultsModalProps {
  open: boolean;
...
function buildResultStats(
  statistics: SendMesGroupCampaignStatistics,
  resultsTotal: number,
) {
  return [
    {
      label: "Thành công",
      value: statistics.mess_group_success ?? statistics.success ?? 0,
      valueClass: "text-success-600 dark:text-success-400",
    },
    {
      label: "Thất bại",
      value: statistics.mess_group_failure ?? statistics.failed ?? 0,
...
export default function SendMesGroupResultsModal({
  open,
  accounts,
  campaignName,
  onClose,
}: SendMesGroupResultsModalProps) {
  const results = useZaloSendMesGroupCampaignStore((s) => s.results);
  const resultsSelectedIds = useZaloSendMesGroupCampaignStore((s) => s.resultsSelectedIds);
  const resultsPage = useZaloSendMesGroupCampaignStore((s) => s.resultsPage);
  const resultsPerPage = useZaloSendMesGroupCampaignStore((s) => s.resultsPerPage);
  const resultsTotal = useZaloSendMesGroupCampaignStore((s) => s.resultsTotal);
  const resultsLoading = useZaloSendMesGroupCampaignStore((s) => s.resultsLoading);
  const statistics = useZaloSendMesGroupCampaignStore((s) => s.statistics);
  const toggleResultSelected = useZaloSendMesGroupCampaignStore((s) => s.toggleResultSelected);
  const toggleSelectAllResults = useZaloSendMesGroupCampaignStore((s) => s.toggleSelectAllResults);
  const setResultsPage = useZaloSendMesGroupCampaignStore((s) => s.setResultsPage);
  const setResultsPerPage = useZaloSendMesGroupCampaignStore((s) => s.setResultsPerPage);
  const deleteSelectedResults = useZaloSendMesGroupCampaignStore((s) => s.deleteSelectedResults);
  const refreshResults = useZaloSendMesGroupCampaignStore((s) => s.refreshResults);
                    ...
                    return (
                      <TableRow key={row.id}>
                          ...
                          <div className="flex items-start gap-2">
                            {thumb ? (
                              <span className="relative block h-10 w-10 shrink-0 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
                                <Image
                                  src={getSendMesGroupMediaUrl(thumb)}
                                  alt=""
                                  fill
                                  unoptimized
                        ...
                        <TableCell className="px-4 py-3 text-theme-sm">
                          {(() => {
                            const status = formatSendMesGroupResultStatus(
                              row.status,
                            );
                            return (
