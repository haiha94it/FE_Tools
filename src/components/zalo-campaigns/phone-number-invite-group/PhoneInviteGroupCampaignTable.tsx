...
interface PhoneInviteGroupCampaignTableProps {
  ...
  onEdit: (campaign: PhoneInviteGroupCampaign) => void;
  onCopy: (campaign: PhoneInviteGroupCampaign) => void;
  onResults: (campaign: PhoneInviteGroupCampaign) => void;
  onDelete: (campaign: PhoneInviteGroupCampaign) => void;
}

const headerClass =
  "px-4 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400";

export default function PhoneInviteGroupCampaignTable({
  campaigns,
  selectedIds,
  loading,
  actionLoading,
  ...
  onResults,
  onDelete,
}: PhoneInviteGroupCampaignTableProps) {
  ...
  if (campaigns.length === 0) {
    return (
      <p className="py-16 text-center text-sm text-gray-500 dark:text-gray-400">
        Chưa có kịch bản mời SĐT vào nhóm. Bấm &quot;Thêm kịch bản&quot; để tạo mới.
      </p>
    );
  }
            ...
            {campaigns.map((campaign, index) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="px-4 py-3 text-theme-sm text-gray-600 dark:text-gray-300">
                    {phoneCount === 0
                      ? "Chưa có số điện thoại nào được gán"
