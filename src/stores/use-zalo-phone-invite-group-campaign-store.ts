...
interface PhoneInviteGroupCampaignState {
  campaigns: PhoneInviteGroupCampaign[];
  ...
  results: PhoneInviteGroupCampaignResult[];
  ...
  createOrEditCampaign: (payload: PhoneInviteGroupCampaignFormPayload) => Promise<void>;
...
export const useZaloPhoneInviteGroupCampaignStore = create<PhoneInviteGroupCampaignState>(
  (set, get) => ({
    campaigns: [],
    accounts: [],
    selectedIds: [],
    ...
    fetchCampaigns: async (options) => {
      ...
      return dedupeInflight(
        `zalo-phone-invite-group-campaign:fetchCampaigns:${silent ? "silent" : "full"}`,
        async () => {
        if (!silent) {
          set({ loading: true, error: null });
        }
        try {
          const campaigns = await zaloPhoneInviteGroupCampaignService.listCampaigns();
          set({
    ...
    fetchAccounts: async () => {
      return dedupeInflight(`zalo-phone-invite-group-campaign:fetchAccounts`, async () => {
    ...
    createOrEditCampaign: async (payload) => {
      set({ saving: true });
      try {
        await zaloPhoneInviteGroupCampaignService.createOrEditCampaign(payload);
    ...
    deleteCampaign: async (id) => {
      ...
      try {
        await zaloPhoneInviteGroupCampaignService.deleteCampaign(id);
        set((state) => ({
    ...
    copyCampaign: async (id, name) => {
      set({ actionLoading: true });
      try {
        await zaloPhoneInviteGroupCampaignService.copyCampaign(id, name);
    ...
    deleteSelectedResults: async () => {
      ...
      try {
        await zaloPhoneInviteGroupCampaignService.deleteResults(
          resultsCampaignId,
          resultsSelectedIds,
        );
        set((state) => ({
    ...
    refreshResults: async (options) => {
      ...
      try {
        const [pageData, statistics] = await Promise.all([
          zaloPhoneInviteGroupCampaignService.fetchResults({
