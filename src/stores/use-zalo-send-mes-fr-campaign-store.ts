...
interface SendMesFrCampaignState {
  campaigns: SendMesFrCampaign[];
  ...
  results: SendMesFrCampaignResult[];
  resultsSelectedIds: number[];
  resultsPage: number;
  resultsPerPage: number;
  resultsTotal: number;
  resultsLoading: boolean;
  statistics: SendMesFrCampaignStatistics;
  ...
  createOrEditCampaign: (payload: SendMesFrCampaignFormPayload) => Promise<void>;
...
export const useZaloSendMesFrCampaignStore = create<SendMesFrCampaignState>(
  (set, get) => ({
    campaigns: [],
    accounts: [],
    selectedIds: [],
    ...
    createOrEditCampaign: async (payload) => {
      set({ saving: true });
      try {
        await zaloSendMesFrCampaignService.createOrEditCampaign(payload);
    ...
    deleteCampaign: async (id) => {
      ...
      try {
        await zaloSendMesFrCampaignService.deleteCampaign(id);
        set((state) => ({
    ...
    copyCampaign: async (id, name) => {
      set({ actionLoading: true });
      try {
        await zaloSendMesFrCampaignService.copyCampaign(id, name);
    ...
    refreshResults: async (options) => {
      ...
      try {
        const [pageData, statistics] = await Promise.all([
          zaloSendMesFrCampaignService.fetchResults({
            categoryId: resultsCampaignId,
            page: resultsPage,
            perPage: resultsPerPage,
          }),
          zaloSendMesFrCampaignService.fetchStatistics(resultsCampaignId),
        ]);
        set({
