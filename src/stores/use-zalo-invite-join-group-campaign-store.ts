...
interface InviteJoinGroupCampaignState {
  campaigns: InviteJoinGroupCampaign[];
  ...
  results: InviteJoinGroupCampaignResult[];
  resultsSelectedIds: number[];
  resultsPage: number;
  resultsPerPage: number;
  resultsTotal: number;
  resultsLoading: boolean;
  statistics: InviteJoinGroupCampaignStatistics;
  failedPhones: string[];
  ...
  createOrEditCampaign: (payload: InviteJoinGroupCampaignFormPayload) => Promise<void>;
...
export const useZaloInviteJoinGroupCampaignStore = create<InviteJoinGroupCampaignState>(
  (set, get) => ({
    campaigns: [],
    accounts: [],
    selectedIds: [],
    ...
    fetchCampaigns: async (options) => {
      ...
      return dedupeInflight(
        `zalo-invite-join-group-campaign:fetchCampaigns:${silent ? "silent" : "full"}`,
        async () => {
        if (!silent) {
          set({ loading: true, error: null });
        }
        try {
          const campaigns = await zaloInviteJoinGroupCampaignService.listCampaigns();
          set({
    ...
    fetchAccounts: async () => {
      return dedupeInflight(`zalo-invite-join-group-campaign:fetchAccounts`, async () => {
    ...
    createOrEditCampaign: async (payload) => {
      set({ saving: true });
      try {
        await zaloInviteJoinGroupCampaignService.createOrEditCampaign(payload);
    ...
    deleteCampaign: async (id) => {
      ...
      try {
        await zaloInviteJoinGroupCampaignService.deleteCampaign(id);
        set((state) => ({
    ...
    copyCampaign: async (id, name) => {
      set({ actionLoading: true });
      try {
        await zaloInviteJoinGroupCampaignService.copyCampaign(id, name);
    ...
    startCampaigns: async (type) => {
      ...
      set({ actionLoading: true });
      try {
        await zaloInviteJoinGroupCampaignService.startCampaigns(ids, type);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    stopCampaigns: async () => {
      ...
      set({ actionLoading: true });
      try {
        await zaloInviteJoinGroupCampaignService.stopCampaigns(ids);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
