  ...
  (set, get) => ({
    ...
    fetchCampaigns: async (options) => {
      ...
      return dedupeInflight(
        `zalo-send-mess-member-gr-campaign:fetchCampaigns:${silent ? "silent" : "full"}`,
        async () => {
    ...
    fetchAccounts: async () => {
      return dedupeInflight(`zalo-send-mess-member-gr-campaign:fetchAccounts`, async () => {
    ...
    startCampaigns: async (type) => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMessMemberGrCampaignService.startCampaigns(ids, type);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    stopCampaigns: async () => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMessMemberGrCampaignService.stopCampaigns(ids);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    deleteSelectedResults: async () => {
      ...
      try {
        await zaloSendMessMemberGrCampaignService.deleteResults(
          resultsCampaignId,
          resultsSelectedIds,
        );
        set((state) => ({
