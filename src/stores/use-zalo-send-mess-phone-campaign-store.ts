...
export const useZaloSendMessPhoneCampaignStore = create<SendMessPhoneCampaignState>(
  (set, get) => ({
    ...
    startCampaigns: async (type) => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMessPhoneCampaignService.startCampaigns(selectedIds, type);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    stopCampaigns: async () => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMessPhoneCampaignService.stopCampaigns(selectedIds);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    deleteSelectedResults: async () => {
      ...
      try {
        await zaloSendMessPhoneCampaignService.deleteResults(resultsSelectedIds);
        set((state) => ({
