...
export const useZaloSendMesGroupCampaignStore = create<SendMesGroupCampaignState>(
  (set, get) => ({
    ...
    startCampaigns: async (type) => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMesGroupCampaignService.startCampaigns(selectedIds, type);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    stopCampaigns: async () => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMesGroupCampaignService.stopCampaigns(selectedIds);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    deleteSelectedResults: async () => {
      ...
      try {
        await zaloSendMesGroupCampaignService.deleteResults(resultsSelectedIds);
        set((state) => ({
