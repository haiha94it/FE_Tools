...
interface SendMesGroupCampaignState {
  campaigns: SendMesGroupCampaign[];
  ...
  results: SendMesGroupCampaignResult[];
  ...
  createOrEditCampaign: (payload: SendMesGroupCampaignFormPayload) => Promise<void>;
...
export const useZaloSendMesGroupCampaignStore = create<SendMesGroupCampaignState>(
  (set, get) => ({
    campaigns: [],
    accounts: [],
    selectedIds: [],
    ...
    fetchCampaigns: async (options) => {
      ...
      return dedupeInflight(
        `zalo-send-mes-group-campaign:fetchCampaigns:${silent ? "silent" : "full"}`,
        async () => {
        if (!silent) set({ loading: true, error: null });
        try {
          const campaigns = await zaloSendMesGroupCampaignService.listCampaigns();
          set({
    ...
    fetchAccounts: async () => {
      return dedupeInflight(`zalo-send-mes-group-campaign:fetchAccounts`, async () => {
    ...
    createOrEditCampaign: async (payload) => {
      set({ saving: true });
      try {
        await zaloSendMesGroupCampaignService.createOrEditCampaign(payload);
    ...
    deleteCampaign: async (id) => {
      ...
      try {
        await zaloSendMesGroupCampaignService.deleteCampaign(id);
        set((state) => ({
    ...
    copyCampaign: async (id, name) => {
      set({ actionLoading: true });
      try {
        await zaloSendMesGroupCampaignService.copyCampaign(id, name);
    ...
    startCampaigns: async (type) => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMesGroupCampaignService.startCampaigns(ids, type);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    stopCampaigns: async () => {
      ...
      set({ actionLoading: true });
      try {
        await zaloSendMesGroupCampaignService.stopCampaigns(ids);
        set({ actionLoading: false });
        await get().fetchCampaigns({ silent: true });
      } catch (error) {
        set({ actionLoading: false });
    ...
    deleteSelectedResults: async () => {
      ...
      try {
        await zaloSendMesGroupCampaignService.deleteResults(
          resultsCampaignId,
          resultsSelectedIds,
        );
        set((state) => ({
    ...
    refreshResults: async (options) => {
      ...
      try {
        const [pageData, statistics] = await Promise.all([
          zaloSendMesGroupCampaignService.fetchResults({
            categoryId: resultsCampaignId,
            page: resultsPage,
            perPage: resultsPerPage,
          }),
          zaloSendMesGroupCampaignService.fetchStatistics(resultsCampaignId),
        ]);
        set({
