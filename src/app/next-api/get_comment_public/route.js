...
export async function POST(req) {
    try {
        ...
        const paginatedResults = allComments.slice(startIndex, endIndex);

        // Tạo cấu trúc dữ liệu phân trang
        const paginatedResponse = {
            count: totalComments,
            next: endIndex < totalComments ? `/next-api/get_comment_public?page=${page + 1}&number_per_page=${number_per_page}` : null,
