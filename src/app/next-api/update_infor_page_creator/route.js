...
export async function POST(request) {
    try {
        const { clientCookie, infor, csrf, type, proxy } = await request.json();
        const agentOptions = getProxyAgent(proxy);

        const bodyRaw = `${type}=${encodeURIComponent(infor)}`;
        const zaloResponse = await axios.post(
            `https://video.zalo.me/v2/public-api/store/${type === "name" ? "update-name" : "update-description"}`,
            bodyRaw,
            {
                headers: {
