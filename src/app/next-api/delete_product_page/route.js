...
export async function POST(request) {
    try {
        const { clientCookie, csrf, itemId, proxy } = await request.json();
        const agentOptions = getProxyAgent(proxy);
        const bodyRaw = `itemId=${encodeURIComponent(itemId)}`;
        const zaloResponse = await axios.post(
            `https://video.zalo.me/v2/public-api/store/delete-store-item`,
            bodyRaw,
            {
                headers: {
