...
export async function POST(request) {
    try {
        const { clientCookie, status, csrf, proxy } = await request.json();
        const agent = new HttpsProxyAgent(proxy);

        const bodyRaw = `status=${encodeURIComponent(status)}`;
        const zaloResponse = await axios.post(
            `https://video.zalo.me/v2/public-api/store/update-status`,
            bodyRaw,
            {
                headers: {
