...
export async function POST(request) {
    try {
        const { clientCookie, csrf, videoIds, proxy } = await request.json();
        const agent = new HttpsProxyAgent(proxy);
        const bodyRaw = `id=${encodeURIComponent(videoIds)}`;
        const zaloResponse = await axios.post(
            `https://video.zalo.me/v2/public-api/playlist/delete`,
            bodyRaw,
            {
                headers: {
