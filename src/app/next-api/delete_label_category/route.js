...
export async function POST(request) {
    try {
        const { clientCookie, csrf, videoId, proxy, ctaType, type } = await request.json();
        const agentOptions = getProxyAgent(proxy);
        const bodyRaw = `videoId=${encodeURIComponent(videoId)}${ctaType ? `&ctaType=${encodeURIComponent(ctaType)}` : ""}`;
        const zaloResponse = await axios.post(
            `https://video.zalo.me/v2/public-api/cta-video/${type}`,
            bodyRaw,
            {
                headers: {
